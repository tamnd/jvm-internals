...
A playground is the same file format with a different job. A lesson asks a question, makes the reader commit to an answer, measures, and grades. A playground hands over a working apparatus and gets out of the way, and the first one is the class file playground, where a reader writes the fields of a class file in the order the specification lists them and then asks `javap`, this kernel's VM and a fresh JVM what they made of it.

```
playgrounds/classfile/
  playground.py   the source, and the only file a human edits
notebooks/playgrounds/
  classfile.ipynb generated by tools/build.py, committed, never hand edited
```

The front matter is a lesson's, plus `kind: playground`. The filename and the kind have to agree, so a `playground.py` saying `kind: lesson` fails the build and so does the reverse. That rule exists because the kind decides which checks are switched off, and a file that quietly claimed the wrong one would be a lesson with its rules disabled.

Three of the rules in `build.py check` do not apply to a playground. There is no prediction gate minimum, because a gate is a promise that the page knows what the reader is about to see and a page whose whole point is that you change the numbers does not. There is no grader, because there is nothing to be right about. And the cap of 24 tier 0 code cells is off, because the cap protects a lesson's attention budget and a playground is meant to be scrolled through and poked at rather than read once. Everything else still applies: unique cell ids, the bootstrap first, the pin agreeing with `docs/pin.json`, the prose rules over the markdown cells, the citation ledger, and the byte identical rebuild.

`tools/test_classfile_playground.py` takes the code cells out of the committed notebook, runs them in a real jshell on the pinned JDK, and checks the output, which is the only way to test the claim a playground actually makes. It runs in the `graders` CI job, because it needs the pinned JDK for the same reason the lesson graders do.
...
A lesson with `status: draft` in its front matter is exempt from the prediction gate rule and the grader rule, which is what lets a work in progress live on a branch without turning CI red for a week. A playground is exempt from those two and from the tier 0 cell cap, for the reasons above, and everything else on that list is checked in a playground exactly as it is in a lesson.
...
Ninety three tests in the first three, standard library only. The two that matter most are `test_three_builds_are_byte_identical` and `test_check_catches_a_one_character_edit`, because those two are the promise the pipeline rests on.

The fourth needs a JDK, because most of it runs the playground's own notebook rather than reading it. Without one it says so and skips, which is why CI runs it in the job that installs the pin and fails on a skip.
